...
//					val = cell.getCellFormula();
	 ...
	 * @param sortSeq 数组下标
	 */
	public <E> List<E> getDataList(Class<E> cls, final int sortSeq) throws InstantiationException, IllegalAccessException{
		...
		for (Field f : fs){
			...
			if (ef != null && (ef.type()==0 || ef.type()==2) && ef.sort()[sortSeq]>=0){
...
//		}
		...
		for (int i = this.getDataRowNum(); i < this.getLastDataRowNum(); i++) {
			...
			for (int j = 0; j < annotationList.size(); j++) {
				...
				Object val = this.getCellValue(row, ef.sort()[sortSeq]);
				if (val != null){
					...
					try {
						...
						log.info("Get cell value ["+i+","+ef.sort()[sortSeq]+"] error: " + ex.toString());
						val = null;
					}
					// set entity value
					if (os[1] instanceof Field){
