...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/act/dynamic/startForm?procDefId=${formData.processDefinition.id}">${formData.processDefinition.name}</a></li>
	</ul><br/>
				...
				<div class="controls">
					<c:choose>
						<c:when test="${property.type.name=='string' || property.type.name=='long'}">
							<input id="${property.id}" name="fp_${property.id}" type="text" <c:if test="${!property.writable}">readonly</c:if> class="<c:if test="${property.required}">required</c:if>" />
						</c:when>
						<c:when test="${property.type.name=='enum'}">
							<select id="${property.id}" name="fp_${property.id}" <c:if test="${!property.writable}">readonly</c:if> class="<c:if test="${property.required}">required</c:if>" />
